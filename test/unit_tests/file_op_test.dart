...
class MemoryOverride extends IOOverrides {
  final MemoryFileSystem _mfs =
      MemoryFileSystem.test(style: FileSystemStyle.posix);
...
void main() {
  ...
  group("Extract operation", () {
    ExtractOperation eop = container
        .read(operationProvider)[OperationType.extract]! as ExtractOperation;
    test("Extracts files to root folder", () {
      IOOverrides.runWithIOOverrides(() async {
        ...
        // Check whether the FileOperationResults were being emitted correctly
        await expectLater(
          eop.extractFiles(
              selectedFiles: filePaths, rootPath: "/root/", dryRun: false),
          emitsInOrder([
            for (String filePath in filePaths)
              predicate<FileOperationResult>((result) {
                return result.rootPath == "/root/" &&
                    ...
                    result.fileTarget ==
                        filePath.replaceFirst(folderRegex, "") &&
        ...
        // Check whether all files were moved to the root folder
        for (String oldFilePath in filePaths) {
          String newFilePath = oldFilePath.replaceAll(folderRegex, "");
          expect(File(newFilePath).existsSync(), isTrue,
              reason: "$newFilePath does not exist!");
        }

        // Check whether old folders are empty now
        filePaths.map((String filepath) => dirname(filepath)).toSet().forEach(
            (String folderPath) =>
                expect(Directory(folderPath).listSync(), isEmpty));
      }, memoryOverride);
    });
        ...
        () async {
          ...
          for (String oldFilePath in selectedFilePaths) {
            expect(
              ...
              isFalse,
              reason: "Selected file $oldFilePath must not exist!",
            );
            String newFilePath = oldFilePath.replaceAll(folderRegex, "");
            expect(
              File(newFilePath).existsSync(),
              isTrue,
              reason: "Selected file $newFilePath does not exist!",
            );
          }
          ...
          for (String oldFilePath in unselectedFilePaths) {
            expect(
              File(oldFilePath).existsSync(),
              isTrue,
              reason: "Unselected file $oldFilePath does not exist!",
            );
            String newFilePath = oldFilePath.replaceAll(folderRegex, "");
            expect(
              File(newFilePath).existsSync(),
              isFalse,
              reason: "Unselected file $newFilePath must exist!",
            );
          }
        },
        memoryOverride,
